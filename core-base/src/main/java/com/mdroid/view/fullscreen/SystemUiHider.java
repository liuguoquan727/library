 ...
 * {@link #getInstance} will return a {@link SystemUiHiderJellyBean} instance,
...
public abstract class SystemUiHider {
  ...
  protected SystemUiHider(Activity activity, int flags) {
    mActivity = activity;
    mAnchorView = activity.getWindow().getDecorView();
    mFlags = flags;
  }
   ...
   * {@link SystemUiHiderBase} or {@link SystemUiHiderJellyBean} depending on
   * the device.
   *
  ...
  public static SystemUiHider getInstance(Activity activity, int flags) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
      return new SystemUiHiderJellyBean(activity, flags);
    } else {
      return new SystemUiHiderBase(activity, flags);
    }
  }
