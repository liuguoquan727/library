...
public class Linkify {
   ...
   */
  public static final boolean addLinks(Spannable text, int mask, URLSpan.SpanCreator creator) {
    if (mask == 0) {
      return false;
    }
    ...
    if ((mask & WEB_URLS) != 0) {
      gatherLinks(links, text, SpanUtils.URL, new String[] { "http://", "https://", "rtsp://" },
          sUrlMatchFilter, null);
    }

    if ((mask & EMAIL_ADDRESSES) != 0) {
      gatherLinks(links, text, SpanUtils.EMAIL_ADDRESS, new String[] { "mailto:" }, null, null);
    }

    if ((mask & PHONE_NUMBERS) != 0) {
      gatherLinks(links, text, SpanUtils.TEL, new String[] { "tel:" }, null, null);
    }
    ...
    for (LinkSpec link : links) {
      applyLink(link.url, link.start, link.end, text, creator);
    }
  ...
  public static final boolean addLinks(TextView text, int mask) {
    ...
    if (t instanceof Spannable) {
      if (addLinks((Spannable) t, mask, null)) {
  ...
  public static final void addLinks(TextView text, Pattern p, String scheme,
      MatchFilter matchFilter, TransformFilter transformFilter, URLSpan.SpanCreator spanCreator) {
  ...
  public static final void applyLink(String url, int start, int end, Spannable text,
      URLSpan.SpanCreator spanCreator) {
    URLSpan span;
    if (spanCreator != null) {
  ...
  private static final void gatherLinks(ArrayList<LinkSpec> links, Spannable s, Pattern pattern,
