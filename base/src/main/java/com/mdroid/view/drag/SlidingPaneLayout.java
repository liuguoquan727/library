 ...
 * pattern instead. ({@link android.support.v4.widget.DrawerLayout DrawerLayout} implements this
 * pattern.)</p>
 * <p>
...
public class SlidingPaneLayout extends ViewGroup {
   ...
   */
  private static final int DEFAULT_OVERHANG_SIZE = 132; // dp;
  /**
  ...
  private float mSlideableViewScale = DEFAULT_SCALE;
  /**
        ...
        if (canSlide && child != mSlideableView) {
          if (lp.width < 0 && (measuredWidth > fixedPanelWidthLimit || lp.weight > 0)) {
            ...
            final int childWidthSpec = MeasureSpec.makeMeasureSpec(
                fixedPanelWidthLimit + (int) (widthSize * mSlideableViewScale / 2 + 0.5),
                MeasureSpec.EXACTLY);
            child.measure(childWidthSpec, childHeightSpec);
          }
        } else if (lp.weight > 0) {
  ...
  @Override protected void onLayout(boolean changed, int l, int t, int r, int b) {
    ...
    for (int i = 0; i < childCount; i++) {
      ...
      if (lp.slideable) {
        ...
        final int range =
            Math.min(nextXStart, width - paddingEnd - mOverhangSize) - xStart - margin;
        mSlideRange = range;
        final int lpMargin = isLayoutRtl ? lp.rightMargin : lp.leftMargin;
        lp.dimWhenOffset = true;// xStart + lpMargin + range + childWidth / 2 > width - paddingEnd;
  ...
  @Override public boolean onInterceptTouchEvent(MotionEvent ev) {
    ...
    if (!mCanSlide && action == MotionEvent.ACTION_DOWN && getChildCount() > 1) {
      ...
      if (secondChild != null) {
        mPreservedOpenState =
            !mDragHelper.isViewUnder(secondChild, (int) ev.getX(), (int) ev.getY());
      }
    }
    ...
    switch (action & MotionEventCompat.ACTION_MASK) {
      ...
      case MotionEvent.ACTION_UP: {
        if (isDimmed(mSlideableView)) {
          ...
          final int slop = mDragHelper.getTouchSlop();
          if (dx * dx + dy * dy < slop * slop && mDragHelper.isViewUnder(mSlideableView, (int) x,
              (int) y)) {
  ...
  private void onPanelDragged(int newLeft) {
    ...
    getBackground().setColorFilter(evaluate(mSlideOffset, Color.BLACK, Color.TRANSPARENT),
  ...
  private void parallaxOtherViews(float slideOffset) {
    ...
    for (int i = 0; i < childCount; i++) {
      ...
      if (v == mSlideableView) {
        float f1 = 1 - mSlideOffset * mSlideableViewScale;
        ViewHelper.setScaleX(v, f1);
        ViewHelper.setScaleY(v, f1);
      } else {
        ...
        if (dimViews) {
          dimChildView(v, isLayoutRtl ? mParallaxOffset - 1 : 1 - mParallaxOffset,
              mCoveredFadeColor);
        }

        ViewHelper.setTranslationX(v, -v.getWidth() / 2.3f + v.getWidth() / 2.3f * mParallaxOffset);
        ViewHelper.setScaleX(v, 0.5f + 0.5f * mParallaxOffset);
        ViewHelper.setScaleY(v, 0.5f + 0.5f * mParallaxOffset);
        ViewHelper.setAlpha(v, mParallaxOffset);
      }
    }
  }
  ...
  static class SlidingPanelLayoutImplJB extends SlidingPanelLayoutImplBase {
    ...
    @Override public void invalidateChildRegion(SlidingPaneLayout parent, View child) {
  ...
  private class DragHelperCallback extends ViewDragHelper.Callback {
    ...
    @Override public int clampViewPositionHorizontal(View child, int left, int dx) {
      ...
      if (isLayoutRtlSupport()) {
        int startBound =
            getWidth() - (getPaddingRight() + lp.rightMargin + mSlideableView.getWidth());
